...
- `void setEqRange(int8_t val, const TDA7439_bands range);` - установка полосы эквалайзера;
  ...
  - `range` - полоса эквалайзера, BASS (низкие частоты), MIDDLE (средние частоты), TREBLE (высокие частоты);

- `void setTimbre(int8_t bass, int8_t middle, int8_t treble);` - установка тембра по всем трем полосам одновременно;
